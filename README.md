...
# Harness API Batch 16

Batch 16 adds the first application shell above the Batch 14/15 runtime and durable storage layers.

- `harness-config` requires `schema_version = 1`, loads `harness.toml`, resolves config-relative paths, and compiles provider/profile declarations into the existing Runtime composition API.
- `harness-cli` produces the `harness` executable with `config check`, `session create`, `run`, and `inspect` commands.
- CLI-generated Session/Agent/Event/Message identifiers use UUID v4 with the existing opaque Harness prefixes.
- `config check` is static: it parses and validates composition input without starting Provider processes. Dynamic Provider manifest/model/tool compatibility is still verified by `HarnessRuntimeBuilder::build()`.
- Batch 16 supports only the existing `allow-all` policy in file configuration and the existing object-level Tool argument validator. Approval UI, JSON Schema execution, credentials, daemon/server mode, and RuntimeEvent observability remain deferred.

A reference configuration is provided as `harness.example.toml`.

```bash
cp harness.example.toml harness.toml
cargo run -p harness-cli --bin harness -- config check
SESSION_ID=$(cargo run -q -p harness-cli --bin harness -- session create)
cargo run -p harness-cli --bin harness -- run "$SESSION_ID"
cargo run -p harness-cli --bin harness -- inspect "$SESSION_ID"
```
